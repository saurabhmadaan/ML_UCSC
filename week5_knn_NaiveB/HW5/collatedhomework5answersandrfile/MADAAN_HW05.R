...
#Perform Naive Bayes on the Sonar Data Set
# 
###############################################################################
setwd("/Users/dcharlu/Documents/UCSC_ML_DataMining/Resources/DataFiles")
rm(list=ls())
...
sonardf <- rbind(STrain,STest)
ncols <- ncol(sonardf)
nxcol <- ncols - 1
names(sonardf)[ncols] = 'yvals'
sonardf[, 'yvals'] = as.factor(sonardf[, 'yvals'])

# train <- sonardf[,1:nxcol]
# yvals <- sonardf[,ncols]

m <- NaiveBayes(yvals ~ ., data = sonardf)
out <- predict(m)
Err <- 1 - sum(out$class == sonardf$yvals)/length(sonardf$yvals)
Err
# [1] 0.2692308

######################## Problem #5 #################################################
# No code required, as the problem only requires to run the example code for
# KfirstNearestNeighbor.R  code
#################################################
